...
> **The app is the source of truth for stage names.** The GHL pipeline stages must match these names exactly. The `agent_pipeline_stage_map` table maps these names to GHL stage IDs.
...
> **Why this matters:** Without the tag check, when an app user drags a card, the push moves the GHL opportunity, which triggers this workflow, which sends a webhook back, which would move the card again — infinite loop. The tag check stops that. It also works the other direction: when a GHL user moves an opportunity, the webhook updates the app, which does NOT add the tag (it's an inbound change), so no echo.
