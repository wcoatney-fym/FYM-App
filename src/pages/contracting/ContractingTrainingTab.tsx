  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="text-sm text-slate-500">Loading training data…</span>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-white text-slate-700 focus:ring-2 focus:ring-blue-400"
              >
                {categories.map((cat) => (
...
function ContentRow({
  ...
  return (
    <Card className={`border-slate-200 transition-colors ${!item.is_active ? 'opacity-60' : ''}`}>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {item.carrier && (
                <span className="text-[10px] text-slate-400">{item.carrier}</span>
              )}
            </div>
          </div>
          ...
          <div className="px-4 pb-3 border-t border-slate-100">
            <div className="pt-3 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
              ...
              <div>
                ...
                <span className="text-slate-700 font-medium">{item.view_count}</span>
              </div>
              {item.has_quiz && (
                <>
                  <div>
                    <span className="text-slate-400 block">Quiz Attempts</span>
              ...
              {!item.has_quiz && (
                <div>
                  <span className="text-slate-400 block">Quiz</span>
...
function EventRow({ event }: { event: PortalTrainingEvent }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-100 rounded-lg">
      <Icon className="w-3.5 h-3.5 text-slate-400 shrink-0" />
      <div className="flex-1 min-w-0">
        <span className="text-xs text-slate-600 font-medium">{info.label}</span>
        {event.content_title && (
