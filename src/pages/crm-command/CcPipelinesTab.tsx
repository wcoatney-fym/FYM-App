    ...
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl overflow-hidden">
        <table className="w-full text-xs"><thead><tr className="border-b border-border/50 bg-secondary/30"><th className="text-left py-3 px-4 font-medium text-muted-foreground">Campaign</th><th className="text-left py-3 px-4 font-medium text-muted-foreground">Source</th><th className="text-right py-3 px-4 font-medium text-muted-foreground">Spend</th><th className="text-right py-3 px-4 font-medium text-muted-foreground">Leads</th><th className="text-right py-3 px-4 font-medium text-muted-foreground">CPL</th><th className="text-left py-3 px-4 font-medium text-muted-foreground">Status</th></tr></thead>
          <tbody>{data.map((row) => (<tr key={row.id} className="border-b border-border/30 hover:bg-secondary/20"><td className="py-3 px-4 font-medium">{row.campaign}</td><td className="py-3 px-4 text-muted-foreground">{row.source}</td><td className="py-3 px-4 text-right">${row.spend.toLocaleString()}</td><td className="py-3 px-4 text-right">{row.leads}</td><td className="py-3 px-4 text-right">${row.cpl.toFixed(2)}</td><td className="py-3 px-4"><span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium', row.status === 'active' ? 'bg-emerald-400/10 text-emerald-400' : row.status === 'paused' ? 'bg-amber-400/10 text-amber-400' : 'bg-slate-400/10 text-slate-400')}>{row.status}</span></td></tr>))}</tbody>
        </table>
      </div>
    </div>
  );
