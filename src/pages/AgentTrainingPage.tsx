...
const FYM_OFFICE_HOURS_URL = 'https://teamfym.zoom.us/j/89124065004?pwd=XAtmDW2hthFYjpPrRNipdgAPC0dJ9S.1';
...
export function AgentTrainingPage() {
  const { user, isFymAdmin, isViewingAs, effectiveAgencyId } = useEffectiveAuth();
  const viewSource = useViewAsStore((s) => s.viewSource);
  const agentId = user?.id ?? null;

  // Show FYM Office Hours only for FYM Direct views:
  // - FYM admin (org-wide, not viewing as)
  // - FYM Direct pivot (viewSource === 'fym-direct')
  // - Agent/manager whose own agency is FYM
  const showOfficeHours =
    (isFymAdmin && !isViewingAs) ||
    viewSource === 'fym-direct' ||
    effectiveAgencyId === FYM_AGENCY_ID;
  ...
  return (
    <>
      ...
      <div className="p-6 max-w-screen-xl mx-auto space-y-6">
        ...
        {showOfficeHours && <FymOfficeHoursBanner />}
