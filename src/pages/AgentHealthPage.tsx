...
// ─── Mock data for demo / no-policy-cache mode ───────────────────────────────
...
export function AgentHealthPage() {
  ...
  // Agents can only view their own page
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (healthRes.data) {
        ...
        // policy_cache not yet populated — show mock
        setHealthData(MOCK_HEALTH);
        setUsingMock(true);
      }

      setPolicies(policiesRes.data ?? (policiesRes.data === null ? MOCK_POLICIES : []));
  ...
  const scoreComponents = [
    {
      ...
      score: h.persistency_score,
      max: 40,
      detail: `${h.retained_count} of ${h.ever_drafted_count} policies reached draft 3+`,
    },
    {
      label: 'Payment Method Mix',
      icon: <CreditCard size={16} />,
      score: h.payment_method_score,
      max: 20,
      detail: 'Monthly EFT/ACH as % of active policies',
    },
    {
      label: 'Contact Recency',
      icon: <Phone size={16} />,
      score: h.contact_recency_score,
      max: 25,
      detail: 'Avg days since last client contact (lower = better)',
    },
    {
      label: 'Product Diversity',
      icon: <LayoutGrid size={16} />,
      score: h.product_diversity_score,
      max: 15,
      detail: 'Balance of HI vs HHC in active book',
    },
  ];
                ...
                {atRiskPolicies.length > 0 && (
                  <>
                    ...
                    <span className="text-red-600 font-medium flex items-center gap-1">
                      <AlertTriangle size={13} /> {atRiskPolicies.length} at risk
                    </span>
                  </>
                )}
              </div>
              ...
              <div className="divide-y divide-slate-100">
                {atRiskPolicies.map((p) => (
                  <div key={p.policy_number} className="flex items-center gap-3 px-4 py-3">
                    ...
                    {p.flag_type && (
                      <Badge className={`text-xs border ${flagColor(p.flag_type)}`}>
                        {flagLabel(p.flag_type)}
                      </Badge>
                    )}
                  </div>
                ))}
              ...
              {policies.map((p) => (
                <div key={p.policy_number} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors">
                  ...
                  <div className="text-right flex-shrink-0 flex items-center gap-2">
                    {p.flag_type && (
                      <Badge className={`text-xs border hidden sm:inline-flex ${flagColor(p.flag_type)}`}>
                        {flagLabel(p.flag_type)}
                      </Badge>
                    )}
                    <div>
