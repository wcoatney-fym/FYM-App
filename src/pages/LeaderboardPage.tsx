...
// Mock leaderboard for demo / mock-data mode
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-4">
        ...
        {/* Personal best card — shown only to agents */}
        {role === 'agent' && myRow && (
            ...
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  ...
                  <span className={`font-bold ${scoreColor(myRow.total_score)}`}>
                    {myRow.total_score}
                  </span>
                </p>
              </div>
            ...
            <div className="divide-y divide-slate-100">
              {rows.map((row) => {
                ...
                return (
                  <div
                    key={row.agent_id}
                    className={`flex items-center gap-4 px-4 py-3 transition-colors ${isMe ? 'bg-blue-50' : 'hover:bg-slate-50'}`}
                  >
                    {/* Rank */}
                    <div className="w-8 text-center flex-shrink-0">
                      {rankBadge(row.agency_rank)}
                    </div>
                    ...
                    {/* Score breakdown — compressed for space */}
                    ...
                    <div className="text-right flex-shrink-0">
                      <span className={`text-lg font-bold ${scoreColor(row.total_score)}`}>
                        {row.total_score}
                      </span>
